...
void ShadowController::initialize() {
    ...
    m_point_shadow_far_plane = settings->far;
...
void ShadowController::begin_draw() {
    ...
    const auto far_plane = glm::max(settings->far, 0.2f);
    m_point_shadow_far_plane = far_plane;
    m_spot_shadow_far_plane = far_plane;
    ...
    m_spot_shadow_map->set_clip_planes(0.1f, far_plane);
...
void ShadowController::render_spot_shadow() const {
    ...
    shader->use();
    shader->set_vec3("uLightPos", get<SettingsController>()->u_slight_pos);
